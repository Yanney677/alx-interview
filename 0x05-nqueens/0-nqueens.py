#!/usr/bin/python3
"""
    N queens puzzle is the challenge of placing N non-attacking queens on an N×N chessboard. A scrpits that solves the N queens problem.
"""
...
def print_board(board):
    """ print_board
    Args:
        board - checks the length of the list in list using the sys.argv[1]
    """
    new_list = []
...
def isSafe(board, row, col, number):
    """ isSafe
    Args:
        board - ckecks the length of the list in list using sys.argv[1]
...
def solveNQUtil(board, col, number):
    ...
    for i in range(number):
        ...
        if (isSafe(board, i, col, number)):
            ...
            board[i][col] = 0  # Back track if solution is not possible
