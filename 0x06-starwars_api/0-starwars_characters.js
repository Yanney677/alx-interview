...
// An API request to get film information from the URL
request(Movie_URL + Movie_Num, async function (err, res, body) {
  if (err) return console.error(err);

  // Parse the response body to get the list of character passed to the URLs
  const charURLList = JSON.parse(body).characters;

  // Iterare through the character URLs and fetch character information
